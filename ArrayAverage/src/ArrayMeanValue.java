...
public class ArrayMeanValue {
    public static double avgArray(Array<Integer> arr)
    {
        double avg=0;
        ...
        return avg/arr.getSize();
    }
    public static int brojDoProsek(Array<Integer> arr) {
        double prosek=avgArray(arr);
        double minRazlika=abs(arr.get(0)-prosek);
        double razlika;
        int broj=0;
        for(int i=0; i<arr.getSize(); i++)
        {
            razlika=abs(arr.get(i)-prosek);
            if(razlika<minRazlika)
            {
                broj=i;
                minRazlika=razlika;
            }
            else if(razlika==minRazlika && arr.get(i)<arr.get(broj))
            {
                broj=i;
            }
        }
